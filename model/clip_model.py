...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, need_limit=False,mask_list=None,need_mae=False,mask_ratio=0.6):
        ...
        if self.proj is not None:
            x = x @ self.proj
        if need_mae:
            return x, atten, mask, ids_restore
        else:
            return x, atten
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text,need_limit=False,mask_list=None,need_MAE=False,mask_ratio=0.6):
        if need_MAE:
            if need_limit:
                image_features, atten_i, mask, ids_restore = self.encode_image(image,need_limit,mask_list,need_MAE,mask_ratio)
            else:
                image_features, atten_i, mask, ids_restore = self.encode_image(image,need_mae=need_MAE,mask_ratio=mask_ratio)
        else:
        ...
        if need_MAE:
            return image_features, atten_i, text_features, atten_t, mask, ids_restore
        else:
